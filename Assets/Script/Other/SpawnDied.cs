...
public class SpawnDied : MonoBehaviour
{
    public Vector3 posFire;
    ...
    private void OnEnable() {
        foreach(Rigidbody rb in listRb){
            ...
            rb.velocity = new Vector3(Random.Range(-5,5),Random.Range(1,3),Random.Range(-5,5));
        }
        coroutine = null;
        coroutine = StartCoroutine(MoveDownAndHide());
    ...
    private void OnDisable() {
        for(int i = 0; i<child.Count;i++){
            ...
            child[i].GetComponent<Collider>().isTrigger = false;
